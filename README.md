...
During development, we can now run the application
using `React-scripts` by running `cd frontend; npm start`, and we'll have all the benefits of rapid application 
...
artifactId to whatever you want. In this example I chose `com.sandbox` and `spring-and-react`. 
Generate the project and unzip it into your project directory. we probably want to initialize git, add a .gitignore
and make your initial commit at this point. 

we should now have a project with a pom that looks something like this: 
...
<project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <description>Testing Devices React Application integrating with Spring Boot</description>
...
```
Let's add a controller called `com.sandbox.springandreact.DevicesController`: 
...
Then, in another window, fetch http://localhost:8080/api/availableDevices with curl or your web browser: 

```
$ curl http://localhost:8080/api/availableDevices
Device List
...
That's great, now have a rest service in Spring Boot up and running!

## Adding React

uses the tool npx which is included with newer versions of `npm`. 

```
$ npx create-react-app frontend

Creating a new React app in /Users/react/git/react-and-spring/frontend.

Installing packages. This might take a couple of minutes.
Installing react, react-dom, and react-scripts...
[...]
Success! Created frontend at /Users/react/git/react-and-spring/frontend
[...]
...
we can now view frontend in the browser.
...
This will open a web browser on desktop, and it should display something like this: 
...
I have to add a `proxy` entry to `frontend/package.json`. This will ensure that the web server at :3000 proxies 
...
the availableDevices service through the frontend server at http://localhost:3000/api/availableDevices

```
$ curl http://localhost:3000/api/availableDevices
...
I like to publish *one* jar file to production, and that jar file should
...
I use `frontend-maven-plugin` in step 1, and `maven-antrun-plugin` in step 2. When we're done, 
...
Open your web browser, and navigate to http://localhost:8080/api/availableDevices
